...
# States
class State(object):
	__slots__ = ("isHead", "isTail", "Main_location")
	...
	def copy_to(self, other: State):
		other.isHead = self.isHead
		other.isTail = self.isTail
		other.Main_location = self.Main_location
	
	def __eq__(self, other):
		return isinstance(other, self.__class__) and self.isHead == other.isHead and self.isTail == other.isTail and self.Main_location == other.Main_location
	...
	def __hash__(self):
		result = 75619
		result = (((101 * result) & 0xFFFFFFFF) + hash(self.isHead)) & 0xFFFFFFFF
		result = (((101 * result) & 0xFFFFFFFF) + hash(self.isTail)) & 0xFFFFFFFF
		result = (((101 * result) & 0xFFFFFFFF) + hash(self.Main_location)) & 0xFFFFFFFF
		return result
	
	def __str__(self):
		result = "("
		result += "isHead = " + str(self.isHead)
		result += ", isTail = " + str(self.isTail)
...
class MainAutomaton(object):
	...
	def __init__(self, network: Network):
		...
		self.transition_counts = [1, 1, 0, 1]
		self.transition_labels = [[1], [0], [], [0]]
		self.branch_counts = [[2], [1], [], [1]]
	...
	def get_guard_value(self, state: State, transition: int) -> bool:
		location = state.Main_location
		if location == 0 or location == 1 or location == 3:
			return True
		else:
			raise IndexError
	...
	def jump(self, state: State, transient: Transient, transition: int, branch: int, assignment_index: int, target_state: State, target_transient: Transient) -> None:
		if assignment_index == 0:
			...
			if location == 0:
				...
				if transition == 0:
					if branch == 0:
						target_state.isTail = True
						target_state.Main_location = 2
			elif location == 3:
				if transition == 0:
					if branch == 0:
						target_state.isHead = True
						target_state.Main_location = 2
			else:
				raise IndexError
...
class Network(object):
	...
	def __init__(self):
		self.network = self
		self.model_type = "mdp"
		self.transition_labels = { 0: "τ", 1: "coinflip" }
		self.sync_vectors = [[0, 0], [1, 1]]
		self.properties = [
			Property("property1", PropertyExpression("<", [PropertyExpression("p_max", [PropertyExpression("eventually", [PropertyExpression("ap", [0])])]), 0.5])),
			Property("property2", PropertyExpression("<=", [PropertyExpression("p_max", [PropertyExpression("eventually", [PropertyExpression("ap", [0])])]), 0.5])),
			Property("property3", PropertyExpression(">", [PropertyExpression("p_min", [PropertyExpression("eventually", [PropertyExpression("ap", [0])])]), 0.5])),
			Property("property4", PropertyExpression(">=", [PropertyExpression("p_min", [PropertyExpression("eventually", [PropertyExpression("ap", [0])])]), 0.5]))
		]
		self.variables = [
			VariableInfo("isHead", None, "bool"),
			VariableInfo("isTail", None, "bool"),
